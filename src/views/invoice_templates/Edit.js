...
function Edit() {
  ...
  const loadTemplate = async () => {
    try {
      ...
      setFormData({
        ...
        class_level: JSON.stringify(template.class_level || {}, null, 2),
  ...
  const loadCategories = async () => {
    try {
      const response = await get("/v1/platform_admin/invoice_categories");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await put(`/v1/platform_admin/invoice_templates/${id}`, payload);
  ...
  return (
    <FormShell
      ...
      subtitle="Update template settings and JSON configuration."
    ...
    >
      <Form onSubmit={handleSubmit}>
        ...
        <AdminButton type="submit" disabled={loading}>
          {loading ? "Saving..." : "Update Invoice Template"}
        </AdminButton>
      </Form>
    </FormShell>
  );
